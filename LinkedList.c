...
bool insertAfter( LinkedList *list, void *valueToFind, void *value )
{
    ...
    else
    {
        ...
        if ( currNode->data != valueToFind )
        {
            fprintf( stderr, "Error: Could not insertBST, "
...
{
    bool found = false;
    if ( isEmptyLL( list ) ) /* if empty, nothing will be found */
    {
        found = false;
    }
    else
...
void *get( LinkedList *list, int index )
{
    ...
    else if ( getLength( list ) < ( index + 1 ) )
    {
        data = NULL;
    }
    else
 ...
 * REMARKS: Will only work for lists that have used insertBST functions to input
...
{
    bool success = true;
    if ( isEmptyLL( list ) ) /* warn user that the list is empty */
    {
